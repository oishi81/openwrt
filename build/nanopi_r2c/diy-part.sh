...
uci set network.lan.gateway='10.10.10.1'                   # 旁路由设置 IPv4 网关（去掉uci前面的#生效）
uci set network.lan.broadcast='10.10.10.255'                # 旁路由设置 IPv4 广播（去掉uci前面的#生效）
...
sed -i 's/PATCHVER:=5.15/PATCHVER:=6.0/g' target/linux/rockchip/Makefile
...
rm -rf rockchip-armv8-friendlyelec_nanopi-r2c.manifest
EOF
